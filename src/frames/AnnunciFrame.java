...
public class AnnunciFrame extends JFrame {
    ...
    // Titolo rimosso su richiesta: niente label principale

        // Pannello sinistro: titolo + search
        JPanel leftHeader = new JPanel(new FlowLayout(FlowLayout.LEFT,8,0));
        leftHeader.setOpaque(false);
    // Barra di ricerca ridotta (meno larga)
    ...
    // Nessuna rimozione necessaria perché il titolo non viene aggiunto
        header.add(leftHeader, BorderLayout.WEST);

        // Pannello destro: filtri e ordine
        ...
        annunciList.setFixedCellHeight(-1); // allow variable height cards
    ...
    // Pannello azioni a destra: Aggiungi annuncio (sempre) + Offri (contextual)
            ...
            if (!e.getValueIsAdjusting()) {
                ...
                if (i>=0 && i < annunciUltimaRicercaRaw.size()) {
                    ...
                    if (isMio) {
                        ...
                        offriBtn.setEnabled(false);
                        offriBtn.setToolTipText("Annuncio Regalo: nessuna offerta necessaria");
                    } else {
    ...
    private String[] buildTipoFiltroModel() {
        ...
        model.add("Tutti"); model.addAll(tipi);
    ...
    private String[] buildCategoriaFiltroModel() {
        ...
        java.util.List<String> model = new java.util.ArrayList<>();
        model.add("Tutte"); model.addAll(cat);
        return model.toArray(new String[0]);
    }
    ...
    private void eseguiRicerca() {
        try {
            ...
            // Nuovo: includi anche i miei annunci
            ...
            if (hasSearch) {
                ...
                for (String rec : filtrati) {
                    // rec formato: ID|Titolo|Tipo|Categoria|Prezzo|Creatore
            ...
            // Ordinamento per prezzo se richiesto (solo su tipo Vendita, altri mantenuti ordine relativo)
            ...
            if (ordSel != null && !"Default".equals(ordSel)) {
                ...
                java.util.Comparator<String> cmpPrezzo = (a,b) -> {
                    String[] pa = a.split("\\|", -1);
                    String[] pb = b.split("\\|", -1);
                    // prezzo alla colonna 5? In record con creatore è index 4
                    ...
                    if (da == null) return 1; // null alla fine
                    if (db == null) return -1;
                    return da.compareTo(db);
                };
                copia.sort(cmpPrezzo);
                if ("Prezzo decrescente".equals(ordSel)) {
            ...
            for (String rec : filtrati) annunciListModel.addElement(rec); // store raw, renderer will format
    ...
    // Crea una JTextArea con placeholder (testo grigio che scompare al focus / primo input)
    private JTextArea createPlaceholderTextArea(String placeholder, int rows, int cols) {
        ...
        area.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            private void update() {
                // Se l'utente ha cancellato tutto mentre non ha focus, ripristiniamo al focusLost
                // Qui non facciamo nulla, la logica è sufficiente con focus events
            }
    ...
    panel.add(new JScrollPane(commentoArea), BorderLayout.CENTER);
        ...
        gc.insets = new Insets(4,4,4,4); gc.anchor = GridBagConstraints.WEST; gc.fill = GridBagConstraints.HORIZONTAL; gc.weightx=1;
        int r=0;
        ...
        if ("Vendita".equalsIgnoreCase(tipo)) {
            gc.gridx=0; gc.gridy=r; extra.add(new JLabel("Prezzo offerto (€):"), gc); gc.gridx=1; extra.add(prezzoField, gc); r++;
            ...
            gc.gridx=0; gc.gridy=r; extra.add(new JLabel("Oggetto da scambiare:"), gc); gc.gridx=1; extra.add(oggettoCombo, gc); r++;
        }
        ...
        if (res == JOptionPane.OK_OPTION) {
            try {
                String prezzoStr = null; String ogg = null;
                if ("Vendita".equalsIgnoreCase(tipo)) {
                ...
                if (PLACEHOLDER_COMMENTO.equals(commento)) commento = ""; // Non salvare il placeholder
    ...
    private void apriDialogNuovoAnnuncio() {
        ...
        int row = 0;
        gc.gridx=0; gc.gridy=row; fields.add(new JLabel("Titolo:"), gc); gc.gridx=1; fields.add(titoloField, gc); row++;
        gc.gridx=0; gc.gridy=row; fields.add(new JLabel("Descrizione:"), gc); gc.gridx=1; gc.fill=GridBagConstraints.BOTH; gc.weighty=1; fields.add(new JScrollPane(descrArea), gc); gc.fill=GridBagConstraints.HORIZONTAL; gc.weighty=0; row++;
        gc.gridx=0; gc.gridy=row; fields.add(new JLabel("Tipo:"), gc); gc.gridx=1; fields.add(tipoBox, gc); row++;
        gc.gridx=0; gc.gridy=row; fields.add(new JLabel("Categoria:"), gc); gc.gridx=1; fields.add(catBox, gc); row++;
        gc.gridx=0; gc.gridy=row; fields.add(prezzoLabel, gc); gc.gridx=1; fields.add(prezzoField, gc); row++;
    gc.gridx=0; gc.gridy=row; fields.add(new JLabel("Oggetto:"), gc); gc.gridx=1; fields.add(oggettoCombo, gc); row++;

        // Visibilità iniziale prezzo
        ...
        if (res == JOptionPane.OK_OPTION) {
            try {
                ...
                if ("Vendita".equalsIgnoreCase(tipo)) {
                    ...
                    try { prezzo = new java.math.BigDecimal(prezzoTxt.replace(",",".")); } catch (NumberFormatException nfe) { throw new IllegalArgumentException("Formato prezzo non valido"); }
                }
                // Mapping string -> enum attraverso Controller utility pubbliche già esistenti
                dto.TipoAnnuncioDTO tipoEnum = null;
                for (dto.TipoAnnuncioDTO t : dto.TipoAnnuncioDTO.values()) if (t.name().equalsIgnoreCase(tipo)) { tipoEnum = t; break; }
                if (tipoEnum == null) throw new IllegalArgumentException("Tipo non valido");
                dto.CategoriaAnnuncioDTO catEnum = null;
                for (dto.CategoriaAnnuncioDTO c : dto.CategoriaAnnuncioDTO.values()) if (c.name().equalsIgnoreCase(categoria)) { catEnum = c; break; }
                if (catEnum == null) throw new IllegalArgumentException("Categoria non valida");
                controller.creaAnnuncio(titolo, descr, catEnum, tipoEnum, prezzo, idOggetto, matricola);
    ...
    /** Renderer "cool" stile card per ogni annuncio */
    private class AnnuncioCardRenderer extends JPanel implements ListCellRenderer<String> {
        ...
        public Component getListCellRendererComponent(JList<? extends String> list, String value, int index, boolean isSelected, boolean cellHasFocus) {
            ...
            String who = isMio ? "you" : creatore;

            // Titolo
            titoloLabel.setText(titolo + ("Vendita".equalsIgnoreCase(tipo) && prezzo!=null && !prezzo.equals("-") && !prezzo.isEmpty() ? "  €"+prezzo : ""));
            ...
            // Prezzo badge solo per vendita con valore
            ...
            if (isMio) {
                badgePanel.add(makeBadge("MIO", new Color(128,0,128)));
            }
